  ...
  try {
    ...
    const criarGraficoPizza = (containerId, dados, labelKey, valueKey) => {
      ...
      dados.forEach((item) => {
        ...
        const cor = getCor(item[labelKey]);
...
// Carregar aniversariantes para o dashboard (semana atual)
async function loadAniversariantesDashboard() {
  ...
  try {
    ...
    // Filtrar apenas aniversariantes da semana atual
    const hoje = new Date();
    ...
    const inicioSemana = new Date(anoHoje, mesHoje, diaHoje - diaSemana);
    const fimSemana = new Date(anoHoje, mesHoje, diaHoje + (6 - diaSemana));

    const aniversariantesSemana = data.filter((item) => {
      ...
      // Verificar se está entre início e fim da semana
      return aniversarioAno >= inicioSemana && aniversarioAno <= fimSemana;
    });
