...
const BLOCOS: { key: keyof FormularioPublicoConfig; titulo: string; descricao: string }[] = [
  ...
  { key: 'endereco', titulo: 'Endereço', descricao: 'Campo único de texto livre.' },
  ...
  { key: 'vida_espiritual', titulo: 'Vida espiritual', descricao: 'Batismos, dom espiritual, pregação, discipulado, ministério.' },
...
export default function FormularioPublicoPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('/api/formulario-publico/config', { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()),
      fetch('/api/congregacoes', { headers: { Authorization: `Bearer ${token}` } }).then(r => r.ok ? r.json() : []),
    ])
      .then(([cfg, congs]: [FormularioPublicoConfig, Cong[]]) => {
        setConfig(cfg)
